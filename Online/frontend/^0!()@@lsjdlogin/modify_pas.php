...
//获取用户信息
$mysql_str = "select id,user_name from {$tb_prefix}_admin_user where user_name = ?  and password = ?";
$query = $pdo->prepare($mysql_str);
$query->execute(array($_SESSION['admin_user_name'],$ya_password));
...
$up_sql = "update {$tb_prefix}_admin_user set password = ? where user_name = ? and password = ?";
$up_sql = $pdo->prepare($up_sql);
$up_res = $up_sql->execute(array($password_ok,$_SESSION['admin_user_name'],$ya_password));
if($up_res){
