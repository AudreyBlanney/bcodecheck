...
$word_content = array(
    ...
    '1.1' => array(
        'grade' => 2,
        'title' => '项目目的',
        'content' => '&nbsp;&nbsp;此次源代码审计工作针对'."{$_SESSION['word_name']}".'，通过分析当前应用系统的源代码与业务流程，从应用系统结构、安全性等方面，检查其脆弱性和缺陷。在明确当前安全现状和需求的情况下，对下一步的编码安全规范性建设进行指导，保障系统上线后安全稳定运行。以此展示Bcodecheck代码审计设备的功能与性能，以及匠迪科技在代码审计领域的相关技术。',
    ),
    '1.2' => array(
        'grade' => 2,
        'title' => '项目概述',
...
&nbsp;&nbsp;本次源代码审计工作主要突出代码编写的缺陷和脆弱性，以OWASP TOP 10为检查依据，针对OWASP统计的问题作重点检查。',
    ),
    '1.3' => array(
        'grade' => 2,
        'title' => '项目范围',
    ...
    '1.3.3' => array(
        'grade' => 3,
        'title' => '审计方法',
        'content' => ' &nbsp;&nbsp;本次源代码审计采用工具测试和人工测试结合的方法进行，依照OWASP TOP 10所披露的脆弱性，根据业务流来检查目标系统的脆弱性、缺陷以及结构上的问题。<br>
本次源代码审计分为三个阶段：<br>
①信息收集<br>
 &nbsp;&nbsp;匠迪Bcodecheck通过源代码安全扫描界面上传业务系统源代码进行信息收集。Bcodecheck会自动分析待审计源码的结构设计、功能模块，输入输出流，以及确定审计检查重点。<br>
②代码安全性分析<br>
 &nbsp;&nbsp;使用Bcodecheck对源代码进行静态扫描；对扫描结果进行人工确认，其中暴露出来的高危问题将对源代码进行人工审计，主要包含以下内容：<br>
...
③问题验证<br>
 &nbsp;&nbsp;对审计工具扫描的结果进行人工整理、分析，得出初步的风险问题，利用Bcodecheck的在线审计工作台载入问题源代码进行人工查找、分析风险代码的上下文，直观的确定代码中存在的安全问题。<br>
 &nbsp;&nbsp;说明：由于客户源代码的实际运行环境，无法对源代码进行动态调试，造成部分安全问题无法验证，我们只是对安全缺陷代码做详细的分析，部分问题可由开发人员参考审计报告进行动态调试。',
    ),
    '2.' => array(
        'grade' => 1,
        'title' => '审计结果',
        'content' => '<p class="MsoNormal" style="text-indent:24.0pt;mso-char-indent-count:2.0">匠迪源代码安全审查系统主要通过<b style="mso-bidi-font-weight:normal"><span style="color:red">总览视图</span></b>和<b style="mso-bidi-font-weight:normal"><span style="color:red">分览视图</span></b>，以总分的方式展现本次代码扫描的结果。审计情况概述如下表所示：</p>',
    ),
    '2.1' => array(
        'grade' => 2,
        'title' => '总览视图',
        'content' =>'<p class="MsoNormal" style="text-indent:24.0pt;mso-char-indent-count:2.0">总览视图主要从整个源代码项目出发，通过文件分类视图、程度分类视图和风险分类是图三个维度展示本次代码审计所发现的问题情况。</p><br>'.
                        $_SESSION['audit_results'],
    ),
    '2.2' => array(
        'grade' => 2,
        'title' => '分览视图',
        'content' =>'<p class="MsoNormal" style="text-indent:24.0pt;mso-char-indent-count:2.0">分览视图将系统源代码审查出来的各类问题，进行分解 详细展示，针对每个问题将提供审查的依据和修复的建议。</p><br>'.$risk_description,
    ),
    '3.' => array(
        'grade' => 1,
        'title' => '审计修复建议',
